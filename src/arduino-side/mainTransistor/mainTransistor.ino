...
void loop() {
  ...
  if (currentMillis - previousMillis >= (paddleMotorStruct.going ? paddleGoingInterval : paddleNotGoingInterval)) {
    ...
    paddleMotorStruct.going = !paddleMotorStruct.going; // Change the going state
  }
...
The last value of the list is a -1 flag, indicating the end for the loop.
*/
...
The second one is the direction of the rotation (0 -> clockwise, 1 -> counterclockwise)
...
void rotateMotor(uint8_t motorIndex, uint8_t rotationDirection, uint8_t times) {
  int motors[2] = {(int)motorIndex, -1};  // Creating a list containg the motor and the -1 flag
...
void throwTrash(TrashType trashType){
  ...
  uint8_t order = (trashType == TRASH_METAL) ? 1 : 0;
